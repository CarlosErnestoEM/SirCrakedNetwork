...
public class ScoreboardAPI {
    ...
    private Objective listhealth;
    public boolean listhealthtab;
    ...
    public ScoreboardAPI(String title, boolean healthname, boolean listheal) {
        this.scoreboard = Bukkit.getScoreboardManager().getNewScoreboard();
        this.title = ChatColor.translateAlternateColorCodes('&', title);
        this.scores = new ConcurrentHashMap<>();
        this.teams = Collections.synchronizedList(Lists.newArrayList());
    ...
    public void update() {
        ...
        if (obj == null) {
            ...
            if (healthname) {
                health = scoreboard.registerNewObjective("showhealth", "health");
                health.setDisplaySlot(DisplaySlot.BELOW_NAME);
                health.setDisplayName(ChatColor.RED + "❤");
            }
            if (listhealthtab) {
                listhealth = scoreboard.registerNewObjective("showhealthlist", "dummy");
                listhealth.setDisplaySlot(DisplaySlot.PLAYER_LIST);
                listhealth.setDisplayName("health");
            }
        }
        ...
        if (listhealthtab) {
            for (Player p : Bukkit.getOnlinePlayers()) {
                listhealth.getScore(p).setScore((int) p.getHealth());
            }
        }
        updated.clear();
    }
