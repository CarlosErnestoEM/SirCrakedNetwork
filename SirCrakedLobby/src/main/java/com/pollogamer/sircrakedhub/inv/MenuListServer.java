...
public class MenuListServer extends GUIPage {
    ...
    public MenuListServer(Player player, String type) {
        ...
        }.runTaskTimer(Principal.plugin, 4, 4);
    }
    ...
    public void buildPage() {
        if (listServer != null) {
            ...
            addButton(new SimpleButton(new ItemStackBuilder(Material.ARROW).setName("§cVolver")).setAction(((plugin1, player1, page) -> new MenuServidores(Principal.plugin, player1))), 36);
            addButton(new SimpleButton(new ItemStackBuilder(Material.ARROW).setName("§cVolver")).setAction(((plugin1, player1, page) -> new MenuServidores(Principal.plugin, player1))), 44);
        } else {
    ...
    public List<String> getListColorized(ListServer listServer, GameServer gameServer) {
        ...
        for (String s : Principal.plugin.getServers().getStringList("guilistserver." + getStage(listServer, gameServer) + ".lore")) {
